...
*   **Mouse Selection:** Dragging on a live terminal selects text — the split drops into read-only scrollback (`Ctrl+Space` resumes), and mouse-up publishes the completed selection to the system clipboard so host terminals that intercept `Command+C` cannot lose it. `Ctrl+C` copies and resumes the live grid; a bare click still just focuses the terminal, and double-click selects words in scrollback. Set `terminal.mouse_drag_selects = false` to make drags on the live grid inert again.
*   **Mouse Forwarding:** Mouse events reach the program inside the terminal only when it enabled mouse reporting (DECSET 1000/1002/1003), and `Shift`+drag bypasses it so you can always select. Set `terminal.mouse_forwarding = "never"` to reserve every mouse event for Fresh's scrollback and selection UI, or `"alt_screen"` to restore the legacy rule (forward everything to any alternate-screen program, no `Shift` bypass).
